...
<<<<<<< HEAD
    "For the Google account to work with this app you need to enable two-factor authentication for Google and generate an app password." : "要在此应用程序中使用Google账户, 您需要为Google启用双重身份验证并生成应用程序密码。",
    "Connecting" : "正在连接",
    "Reconnect Google account" : "重新连接Google账户",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Install Mailvelope browser extension by clicking here" : "点击此处安装 Mailvelope 浏览器扩展",
    ...
    "Add share link from Files" : "从文件添加分享链接",
...
<<<<<<< HEAD
    ...
    "To archive a message please configure an archive mailbox in account settings" : "要归档邮件，请在账户设置中指定一个归档邮箱。",
...
<<<<<<< HEAD
    ...
    "Provisioned account is disabled" : "预配账户已禁用",
    "Please login using a password to enable this account. The current session is using passwordless authentication, e.g. SSO or WebAuthn." : "请使用密码登录以启用此账户。当前会话正在使用无密码身份验证，例如 SSO 或 WebAuthn。",
=======
...
<<<<<<< HEAD
    ...
    "Not supported by the server" : "服务器不支持",
    "{usage} of {limit} used" : "已使用 {usage} / {limit}",
    "The account for {email} and cached email data will be removed from Nextcloud, but not from your email provider." : "{email}的账户和缓存的电子邮件数据将从Nextcloud中删除，但不会从您的电子邮件提供商那里删除。",
    "Remove {email}" : "移除 {email}",
=======
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    "Show all subscribed mailboxes" : "显示所有订阅的邮箱",
    "Show all mailboxes" : "显示所有邮箱",
    "Collapse mailboxes" : "收起邮箱",
    ...
    "Mark all messages of this mailbox as read" : "将此邮箱的全部邮件标记为已读",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Could not copy to \"Sent\" mailbox" : "无法复制到“已发送”邮箱",
    "Mail server error" : "邮件服务器错误",
    "Message could not be sent" : "无法发送邮件",
    "Message deleted" : "邮件已删除",
    "Copy to \"Sent\" Mailbox" : "复制到“已发送”邮箱",
    "Copy to Sent Mailbox" : "复制到已发送邮箱",
    "Phishing email" : "钓鱼邮件",
...
=======
    ...
    "Delete action" : "删除操作",
    "Flag" : "旗帜",
    "The stop action ends all processing" : "停止操作结束所有处理",
    "Sender" : "发件人",
    "Recipient" : "收件人",
    ...
    "matches" : "匹配",
    "Delete test" : "删除测试",
    "Update mail filter" : "更新邮件筛选器",
    "Filter name" : "筛选器名称",
    "If all the conditions are met" : "如果满足所有条件",
    "If any of the conditions are met" : "如果满足任何条件",
    "Tests are applied to incoming emails on your mail server, targeting fields such as subject (the email's subject line), from (the sender), and to (the recipient). You can use the following operators to define conditions for these fields:" : "测试应用于邮件服务器上的传入电子邮件，目标字段如主题（电子邮件的主题行）、发件人和收件人。您可以使用以下运算符为这些字段定义条件：",
    "An exact match. The field must be identical to the provided value." : "完全匹配。此字段必须与提供的值相同。",
    ...
    "Add condition" : "添加条件",
    "Then perform these actions" : "然后执行这些操作",
    "Actions are triggered when the specified tests are true. The following actions are available:" : "当指定的测试为真时，会触发操作。以下操作可用：",
    "Moves the message into a specified folder." : "将邮件移动到指定文件夹中。",
    "Adds a flag to the message." : "在邮件中添加标记。",
    "Halts the execution of the filter script. No further filters with will be processed after this action." : "停止执行筛选器脚本。此操作后，将不会处理其他筛选器。",
    "Add action" : "添加操作",
    "Priority" : "优先级",
    "Enable filter" : "启用筛选器",
    "Save filter" : "保存筛选器",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    "Mail app" : "邮件应用",
    "The mail app allows users to read mails on their IMAP accounts." : "邮件应用允许用户阅读其IMAP账户中的邮件。",
...
<<<<<<< HEAD
    ...
    "Allow additional mail accounts" : "允许其他邮件账户",
    "Allow additional Mail accounts from User Settings" : "允许在用户设置的其他邮件账户",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Gmail allows users to access their email via IMAP. For security reasons this access is only possible with an OAuth 2.0 connection or Google accounts that use two-factor authentication and app passwords." : "Gmail允许用户通过IMAP访问他们的电子邮件。出于安全原因，只有使用OAuth 2.0连接或使用双重身份验证和应用程序密码的Google账户才能进行此访问。",
...
<<<<<<< HEAD
    ...
    "Successfully deleted and deprovisioned accounts for \"{domain}\"" : "成功删除并取消配置了“{domain}”的账户",
    "Error when deleting and deprovisioning accounts for \"{domain}\"" : "删除和取消配置“{domain}”账户时出错",
    "Could not save default classification setting" : "无法保存默认分类设置",
...
<<<<<<< HEAD
    "With the settings above, the app will create account settings in the following way:" : "使用上述设置，该应用将通过以下方式创建账户设置：",
=======
...
<<<<<<< HEAD
    "To add a mail account, please contact your administrator." : "要添加邮件账户，请联系您的管理员。",
    "Next week – {timeLocale}" : "下周 – {timeLocale}",
    "This weekend – {timeLocale}" : "本周末 – {timeLocale}",
    "Tomorrow – {timeLocale}" : "明天 – {timeLocale}",
    "Later today – {timeLocale}" : "今日稍晚 – {timeLocale}",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Sent" : "已发送",
    "Trash" : "废件箱",
    "Connect OAUTH2 account" : "连接OAUTH2账户",
    ...
    "Could not copy message to \"Sent\" mailbox" : "无法将邮件复制到“已发送”邮箱",
...
=======
    "Draft: " : "草稿：",
    "If all tests pass, then the actions will be executed" : "如果全部测试都通过，则将执行操作",
    "If one test pass, then the actions will be executed" : "如果一个测试通过，则将执行操作",
    "Tests" : "测试",
    "New test" : "新测试",
    "Actions" : "操作",
    "New action" : "新操作"
