OC.L10N.register(
    ...
    "Reply-To email: %1$s  is different from the sender email: %2$s" : "返信先の電子メール: %1$s は送信者の電子メール: %2$s と異なります",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Mailto" : "メールアドレス",
    "Register as application for mail links" : "メールリンクのアプリケーションとして新規登録する",
    "Register" : "登録",
    "Privacy and security" : "プライバシーとセキュリティ",
    "Data collection consent" : "データ収集の同意",
    ...
    "Install Mailvelope browser extension by clicking here" : "メールブラウザー拡張機能インストールしたい場合はここをクリックしてください",
    ...
    "No trash mailbox configured" : "ゴミ箱メールボックスが設定されていません",
    ...
    "Collapse mailboxes" : "メールボックスを非表示",
    ...
    "Clear mailbox" : "メールボックスをクリアする",
    "The mailbox and all messages in it will be deleted." : "メールボックスと中にある全てのメッセージが削除されます。",
    "Delete mailbox" : "メールボックスを削除",
    "Delete mailbox {name}" : "メールボックス「{name}」を削除",
    ...
    "Mark all messages of this mailbox as read" : "このフォルダのすべてのメッセージを開封済みにする",
    "Add submailbox" : "サブメールボックスを追加",
    "Rename" : "名前を変更",
    "Move mailbox" : "メールボックスを移動する",
    "Repair mailbox" : "メールボックスを修復する",
    ...
    "Saving new tag name …" : "新しいタグ名を保存中...",
    "Delete tag" : "タグを削除",
    "Set tag" : "タグを設定する",
    "Unset tag" : "タグを削除する",
...
=======
    ...
    "Flag" : "フラグ",
    "The stop action ends all processing" : "stop アクションは、すべての処理を終了します",
    "Recipient" : "受信者",
    ...
    "matches" : "一致",
    "Delete test" : "テストを削除",
    "Update mail filter" : "メールフィルターを更新する",
    "Filter name" : "フィルター名",
    "Tests are applied to incoming emails on your mail server, targeting fields such as subject (the email's subject line), from (the sender), and to (the recipient). You can use the following operators to define conditions for these fields:" : "テストは、Subject（メールの件名）、From（送信者）、To（受信者）などのフィールドを対象として、メールサーバー上の受信メールに適用されます。以下の演算子を使用して、これらのフィールドの条件を定義できます：",
    "An exact match. The field must be identical to the provided value." : "完全一致。フィールドは提供された値と同一でなければなりません。",
    ...
    "Actions are triggered when the specified tests are true. The following actions are available:" : "アクションは、指定されたテストが真であるときにトリガーされる。以下のアクションが利用可能です：",
    "Moves the message into a specified folder." : "メッセージを指定したフォルダーに移動します。",
    "Adds a flag to the message." : "メッセージにフラグを追加する。",
    "Halts the execution of the filter script. No further filters with will be processed after this action." : "フィルタースクリプトの実行を停止する。この操作の後、それ以降のフィルターは処理されません。",
    "Priority" : "優先度",
...
>>>>>>> 5d13aacd343883b2c7ace01db7280a0664c0a6e4
    ...
    "Message copied to \"Sent\" mailbox" : "メッセージが\"送信済み\"メールボックスにコピーされました",
    "Could not copy message to \"Sent\" mailbox" : "メッセージを”送信済み”メールボックスにコピーできませんでした",
...
=======
    "Draft: " : "下書き",
    "If all tests pass, then the actions will be executed" : "すべてのテストがパスすれば、アクションが実行されます",
    "If one test pass, then the actions will be executed" : "テストが1つでもパスすれば、アクションが実行されます",
    "Tests" : "テスト",
    "New test" : "新しいテスト",
    "Actions" : "アクション",
    "New action" : "新しいアクション"
